...
const registerationRequests = async (req, res) => {
  ...
  try {
    ...
    allRequestBusinessOwner = await BusinessOwnersModel.find({
      ...
      .select(
        "_id name last_name username phone_number email country_name state_name city_name address brand_name postal_code work_phone registration_date"
      )
      .sort({ registration_date: -1 });

    const formattedRequests = allRequestBusinessOwner.map((request) => {
      const formattedDate = moment(request.registration_date).format(
        "YYYY-MM-DD"
      );
